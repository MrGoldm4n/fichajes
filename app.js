// =====================================================
// FICHAJES v0.3.0 — app.js
// Telegram Mini App + Apps Script API
// =====================================================
...
async function api(action, data = {}) {
  ...
  const isWrite = ['fichar','corregirFichaje','guardarEmpleado',
    'guardarUbicacion','resolverIncidencia','guardarConfig'].includes(action);
...
window.addEventListener('load', async () => {
  ...
  // Sin contexto de Telegram (test en navegador)
  if (!tgUser && !tg) {
    mostrarNoAuth('Abre la app desde Telegram.');
    return;
  }

  try {
    // 1. Autenticar con el servidor
    const [emp, ubicaciones, config] = await Promise.all([
      api('getEmpleado'),
      api('getUbicaciones'),
      api('getConfig'),
    ]);
    ...
    // 2. ¿Viene de NFC? → localizar ubicación por param
    const urlParams = new URLSearchParams(window.location.search);
    const locParam  = tg?.initDataUnsafe?.start_param || urlParams.get('loc') || '';
    if (locParam) {
      state.ubicacion = ubicaciones.find(u => u.NFC_Param === locParam || u.ID_Ubicacion === locParam) || null;
      if (state.ubicacion)
        document.getElementById('fichar-ubicacion').textContent = '📍 ' + state.ubicacion.Nombre;
    }
    ...
    // 5. Si viene NFC y hay ubicación → fichaje directo
    if (locParam && state.ubicacion) {
      await prepararFichaje(true);
    }

  } catch (err) {
    if (err.message.includes('No autorizado')) {
      mostrarNoAuth(tgUser ? 'Tu ID: ' + tgUser.id : '');
    } else {
      toast('Error: ' + err.message, 'error');
    }
    ocultarSplash('noauth');
    return;
  }

  ocultarSplash('fichar');
  iniciarReloj();
  setupNavegacion();
  setupModales();
});

function mostrarNoAuth(detalle) {
  const box = document.getElementById('tg-id-box');
  if (detalle) box.textContent = detalle;
}

function ocultarSplash(destino) {
...
async function prepararFichaje(autoConfirm) {
  ...
  if (tipo === 'SALIDA') {
    mostrarModalSalida();
  } else if (!autoConfirm && confirmar) {
    mostrarModalConfirmar(tipo);
  } else {
    await ejecutarFichaje({ tipo, esDescanso: false, comentario: '' });
  }
}
...
function mostrarModalConfirmar(tipo) {
  ...
  document.getElementById('modal-subtitulo').textContent   = tipo + ' — ' + hora;
...
function mostrarModalSalida() {
  const hTrab = calcularHorasTrabajadas();
  ...
  document.getElementById('resumen-valor').textContent     = hTrab;
...
async function ejecutarFichaje({ tipo, esDescanso, comentario }) {
  ...
  try {
    ...
    const res = await api('fichar', {
      ...
      metodo:          state.ubicacion ? 'NFC' : 'MINI_APP',
    });
    if (res.ok) {
...
function setupModales() {
  ...
  document.getElementById('modal-confirm').addEventListener('click', async () => {
    const tipo = state.estadoHoy?.proximoTipo || 'ENTRADA';
    await ejecutarFichaje({ tipo, esDescanso: false,
      comentario: document.getElementById('modal-comentario').value.trim() });
  });
  document.getElementById('btn-descanso').addEventListener('click', async () =>
    ejecutarFichaje({ tipo: 'SALIDA', esDescanso: true,
      comentario: document.getElementById('modal-comentario').value.trim() }));
  document.getElementById('btn-salida-ok').addEventListener('click', async () =>
    ejecutarFichaje({ tipo: 'SALIDA', esDescanso: false,
      comentario: document.getElementById('modal-comentario').value.trim() }));
  ...
  document.getElementById('btn-manual').addEventListener('click', async () => {
    ...
    if (state.empleado?.Rol === 'admin') {
      ...
      const sel = document.getElementById('manual-emp');
      sel.innerHTML = state.empleados.map(e =>
  ...
  document.getElementById('emp-cancel').addEventListener('click',
  ...
  document.getElementById('btn-exportar-pdf').addEventListener('click',
    () => toast('PDF en próxima versión', 'info'));
}
...
function iniciarTimerDescanso() {
  ...
  state.timerInterval = setInterval(() => {
    ...
    if (state.timerSeconds <= 0) {
      clearInterval(state.timerInterval);
      toast('⏰ Descanso completado. ¡Vuelve a fichar!', 'warning');
...
async function cargarResumen(numEmp, año) {
  ...
  renderTrimestres(resumen, año);
  renderSemana(resumen.detalleDias);

  if (!document.getElementById('dash-mes').value)
    document.getElementById('dash-mes').value =
      año + '-' + String(new Date().getMonth()+1).padStart(2,'0');
...
function renderTrimestres(resumen, año) {
  ...
  document.getElementById('trimestres-grid').innerHTML = ['Q1','Q2','Q3','Q4'].map((t,i) => {
    ...
    const hTrim = (resumen.detalleDias||[])
      .filter(d=>{ const m=parseInt(d.fecha.split('-')[1]); return m>=ini&&m<=fin; })
      .reduce((a,b)=>a+b.horas,0);
    return `<div class="trim-card ${cls}">
      <div class="trim-label">${t} · ${año}</div>
      <div class="trim-horas">${hTrim.toFixed(1)}h</div>
      <div class="trim-obj">${obj?'Obj: '+obj+'h':'—'}</div>
    </div>`;
  }).join('');
}
...
document.addEventListener('change', async e => {
  if (e.target.id === 'dash-mes') {
    ...
    const año = e.target.value.split('-')[0];
    const res = await api('getResumen', { numEmp, año });
...
function renderCalendario(mesStr, detalleDias) {
  ...
  const porDia = {};
  (detalleDias||[]).forEach(d => { porDia[d.fecha] = d; });
  ...
  for (let d=1; d<=diasMes; d++) {
    ...
    const dia  = porDia[fStr];
    const esFin= [0,6].includes(new Date(año,mes-1,d).getDay());
...
function renderSemana(detalleDias) {
  const hoy   = new Date();
  const dSem  = hoy.getDay()===0?6:hoy.getDay()-1;
...
async function renderFichajesMes(mesStr) {
  ...
  const fichajes  = await api('getFichajes', {
    fechaInicio: año+'-'+mes+'-01',
    fechaFin:    año+'-'+mes+'-31',
  });
  const porDia = {};
  fichajes.forEach(f => { if (!porDia[f.Fecha]) porDia[f.Fecha]=[]; porDia[f.Fecha].push(f); });
  const cont = document.getElementById('lista-fichajes');
  if (!Object.keys(porDia).length) {
    cont.innerHTML = '<div class="empty-state">Sin fichajes en este periodo</div>'; return;
  }
  cont.innerHTML = Object.entries(porDia)
    .sort(([a],[b])=>b.localeCompare(a))
    .map(([fecha,filas]) => {
      const hTrab = calcularHorasDia(filas);
      return `<div class="dia-grupo">
        <div class="dia-titulo">${formatearFecha(fecha)}${hTrab?' · '+hTrab:''}</div>
...
async function cargarEmpleados() {
  ...
  cont.innerHTML = state.empleados.map(e => `
...
async function cargarUbicaciones() {
  ...
  cont.innerHTML = state.ubicaciones.map(u => `
    <div class="admin-card">
      <div class="admin-card-info">
        <div class="admin-card-name">${u.Nombre}</div>
        <div class="admin-card-sub">ID: ${u.ID_Ubicacion} · NFC: ${u.NFC_Param}</div>
        <div class="admin-card-sub nfc-url">
          🔗 ${APPS_SCRIPT_URL}?loc=${u.NFC_Param}
        </div>
      </div>
    </div>`).join('');
}
...
async function navegarA(view) {
  ...
  const mapa = { fichar:'screen-fichar', 'mis-fichajes':'screen-mis-fichajes',
    dashboard:'screen-dashboard', empleados:'screen-empleados',
    ubicaciones:'screen-ubicaciones', incidencias:'screen-incidencias' };
